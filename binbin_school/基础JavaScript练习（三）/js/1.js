...
function parseTextArea(){
	var area_value=area_text.value;
	var area_arr=[];
	area_arr=area_value.split(/,|，|\b|、|\r/);
	// console.log(area_arr);
	area_arr=area_arr.map(function(item){
...
function insertLeft(){
	if(countQueue(60)==true){
		...
		var area_arr=parseTextArea();
		if(pattern(input_value)==true){
			hideElement();
		...
		for (var i = area_arr.length - 1; i >= 0; i--) {
			if(pattern(area_arr[i])==true){
				arr.unshift(area_arr[i]);
				insertLeftSpan(area_arr[i]);
			}
		}
		console.log(arr);
		try{
...
//匹配输入的只有数字
function pattern(test_num){
	
	var pattern_text=/^([1-9]\d|100)$/;
	if(pattern_text.exec(test_num)){
		return true;
	}else{
		return false;
	}
...
function search(){
	...
	for(var i=0;i<arr.length;i++){
		if(pattern.test(arr[i]+"")){
			queue_child[i].style.backgroundColor="blue";
		}
	}
...
function init(){
	...
	input_text.focus();
	area_text=document.getElementById("textarea-input");
	queue=document.getElementById("queue");
	removeSpace();
